...
{
    static TypeId tid = TypeId("ns3::qspu::TypeHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qspu")
...
{
    static TypeId tid = TypeId("ns3::qspu::RreqHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qspu")
...
{
    static TypeId tid = TypeId("ns3::qspu::RrepHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qspu")
...
{
    static TypeId tid = TypeId("ns3::qspu::RrepAckHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qspu")
...
{
    static TypeId tid = TypeId("ns3::qspu::RerrHeader")
                            .SetParent<Header>()
                            .SetGroupName("Qspu")
...
//-----------------------------------------------------------------------------
// QSPU Advertisement Header
//-----------------------------------------------------------------------------

QspuAdHeader::QspuAdHeader ()
  : m_origin (),
    m_gateway (),
    m_adId (0),
    m_hopCount (0),
    m_connectivity10000 (10000),
    m_lifetimeMs (0),
    m_delayUs (0),
    m_lastXcm (0),
    m_lastYcm (0),
...
NS_OBJECT_ENSURE_REGISTERED (QspuAdHeader);

TypeId
QspuAdHeader::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::qspu::QspuAdHeader")
    .SetParent<Header> ()
    .SetGroupName ("Qspu")
    .AddConstructor<QspuAdHeader> ();
  return tid;
}

TypeId
QspuAdHeader::GetInstanceTypeId () const
{
  return GetTypeId ();
}

uint32_t
QspuAdHeader::GetSerializedSize () const
{
  // origin 4
  // gateway 4
  // adId 4
...
void
QspuAdHeader::Serialize (Buffer::Iterator i) const
{
  WriteTo (i, m_origin);
  WriteTo (i, m_gateway);

  i.WriteHtonU32 (m_adId);
  i.WriteU8 (m_hopCount);

  i.WriteHtonU32 (m_connectivity10000);
  i.WriteHtonU32 (m_lifetimeMs);
  i.WriteHtonU32 (m_delayUs);

  i.WriteHtonU32 (static_cast<uint32_t> (m_lastXcm));
  i.WriteHtonU32 (static_cast<uint32_t> (m_lastYcm));
  i.WriteHtonU32 (static_cast<uint32_t> (m_lastZcm));

  i.WriteHtonU32 (static_cast<uint32_t> (m_lastVxCms));
  i.WriteHtonU32 (static_cast<uint32_t> (m_lastVyCms));
  i.WriteHtonU32 (static_cast<uint32_t> (m_lastVzCms));
}

uint32_t
QspuAdHeader::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;

  ReadFrom (i, m_origin);
  ReadFrom (i, m_gateway);

  m_adId = i.ReadNtohU32 ();
  m_hopCount = i.ReadU8 ();

  m_connectivity10000 = i.ReadNtohU32 ();
  m_lifetimeMs = i.ReadNtohU32 ();
  m_delayUs = i.ReadNtohU32 ();

  m_lastXcm = static_cast<int32_t> (i.ReadNtohU32 ());
  m_lastYcm = static_cast<int32_t> (i.ReadNtohU32 ());
  m_lastZcm = static_cast<int32_t> (i.ReadNtohU32 ());

  m_lastVxCms = static_cast<int32_t> (i.ReadNtohU32 ());
  m_lastVyCms = static_cast<int32_t> (i.ReadNtohU32 ());
  m_lastVzCms = static_cast<int32_t> (i.ReadNtohU32 ());
...
void
QspuAdHeader::Print (std::ostream &os) const
{
  os << "QSPU AD "
     << " origin=" << m_origin
     << " gateway=" << m_gateway
     ...
     << " Co10000=" << m_connectivity10000
     << " lifetimeMs=" << m_lifetimeMs
     << " delayUs=" << m_delayUs;
}

bool
QspuAdHeader::operator== (QspuAdHeader const &o) const
{
  return m_origin == o.m_origin
         && m_gateway == o.m_gateway
         && m_adId == o.m_adId
         && m_hopCount == o.m_hopCount
         && m_connectivity10000 == o.m_connectivity10000
         && m_lifetimeMs == o.m_lifetimeMs
         && m_delayUs == o.m_delayUs
         && m_lastXcm == o.m_lastXcm
         && m_lastYcm == o.m_lastYcm
         && m_lastZcm == o.m_lastZcm
         && m_lastVxCms == o.m_lastVxCms
         && m_lastVyCms == o.m_lastVyCms
         && m_lastVzCms == o.m_lastVzCms;
}

std::ostream &
operator<< (std::ostream & os, QspuAdHeader const & h)
{
  h.Print (os);
  return os;
}
