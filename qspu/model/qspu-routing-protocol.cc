 ...
 *
 * Based on
 *      NS-2 QSPU model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      QSPU-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/QSPU-UU
 *
...
#include "qspu-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("QspuRoutingProtocol");

namespace qspu
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for QSPU control traffic
const uint32_t RoutingProtocol::QSPU_PORT = 654;

/**
 * \ingroup qspu
 * \brief Tag used by QSPU implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::qspu::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("Qspu")
...
RoutingProtocol::RoutingProtocol()
    ...
    m_isGateway (false),
m_gatewayNodeId (0),
m_igadInterval (Seconds (4.0)),
m_uavadInterval (Seconds (4.0)),
m_igadTimer (Timer::CANCEL_ON_DESTROY),
m_uavadTimer (Timer::CANCEL_ON_DESTROY),
m_igadId (0),
m_uavadId (0),
m_alpha (0.33),
m_beta (0.33),
m_gamma (0.34),
m_txRange (300.0),
m_dataRateBps (24000000.0),
m_minConnectivity10000 (1),
m_minLifetime (Seconds (0.1)),
m_maxDelay (Seconds (2.0)),
m_maxHopCount (20),
m_dropZeroVelocityUavs (false),
m_currentGateway (),
m_windowBytes (0),
m_loadWindow (Seconds (1.0)),
m_lastLoadUpdate (Seconds (0.0)),
m_loadBps (0.0),
m_loadEwmaAlpha (0.3),
      m_ttlStart(1),
      m_ttlIncrement(2),
...
{
    static TypeId tid =
        TypeId("ns3::qspu::RoutingProtocol")
            ...
            .AddAttribute("EnableBroadcast",
                          ...
                          MakeBooleanChecker())
                          .AddAttribute ("GatewayNodeId",
               "NodeId of the Internet Gateway / GCS.",
               UintegerValue (0),
               MakeUintegerAccessor (&RoutingProtocol::SetGatewayNodeId,
                                      &RoutingProtocol::GetGatewayNodeId),
               MakeUintegerChecker<uint32_t> ())

.AddAttribute ("IgadInterval",
               "IGAD advertisement interval.",
               TimeValue (Seconds (4.0)),
               MakeTimeAccessor (&RoutingProtocol::SetIgadInterval,
                                 &RoutingProtocol::GetIgadInterval),
               MakeTimeChecker ())

.AddAttribute ("UavadInterval",
               "UAVAD reverse advertisement interval.",
               TimeValue (Seconds (4.0)),
               MakeTimeAccessor (&RoutingProtocol::SetUavadInterval,
                                 &RoutingProtocol::GetUavadInterval),
               MakeTimeChecker ())

.AddAttribute ("Alpha",
               "Weight for connectivity.",
               DoubleValue (0.33),
               MakeDoubleAccessor (&RoutingProtocol::SetAlpha,
                                   &RoutingProtocol::GetAlpha),
               MakeDoubleChecker<double> (0.0, 1.0))

.AddAttribute ("Beta",
               "Weight for route lifetime.",
               DoubleValue (0.33),
               MakeDoubleAccessor (&RoutingProtocol::SetBeta,
                                   &RoutingProtocol::GetBeta),
               MakeDoubleChecker<double> (0.0, 1.0))

.AddAttribute ("Gamma",
               "Weight for delay.",
               DoubleValue (0.34),
               MakeDoubleAccessor (&RoutingProtocol::SetGamma,
                                   &RoutingProtocol::GetGamma),
               MakeDoubleChecker<double> (0.0, 1.0))

.AddAttribute ("TxRange",
               "Transmission range used by QSPU metric calculations.",
               DoubleValue (300.0),
               MakeDoubleAccessor (&RoutingProtocol::m_txRange),
               MakeDoubleChecker<double> (1.0))

.AddAttribute ("DataRateBps",
               "Nominal PHY data rate used for QSPU delay estimation.",
               DoubleValue (24000000.0),
               MakeDoubleAccessor (&RoutingProtocol::m_dataRateBps),
               MakeDoubleChecker<double> (1.0))
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No qspu interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // QSPU is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == QSPU_PORT)
                    {
                        // QSPU packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("QSPU does not work with more then one address per each interface.");
    }
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvQspu, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), QSPU_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvQspu, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), QSPU_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("QSPU does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in QSPU operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when QSPU does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // QSPU needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available QSPU interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid QSPU source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by qspu
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(QSPUTYPE_RREQ);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, QSPU_PORT));
}
...
RoutingProtocol::ScheduleRreqRetry(Ipv4Address dst)
{
    ...
    else
    {
        ...
        if (backoffFactor>=30){
        	NS_LOG_WARN("QSPU: backoffFactor too large ("<<backoffFactor<<") for destination "<< dst<< ". Cancelling RREQ retry to avoid integer overflow.");
...
void
RoutingProtocol::RecvQspu(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("QSPU node " << this << " received a QSPU packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("QSPU message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    ...
    case QSPUTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    case QSPUTYPE_IGAD:
  {
    RecvIgad (packet, receiver, sender);
    break;
  }

case QSPUTYPE_UAVAD:
  {
    RecvUavad (packet, receiver, sender);
    break;
  }
    }
}
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(QSPUTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), QSPU_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(QSPUTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), QSPU_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(QSPUTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, QSPU_PORT));
}
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(QSPUTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(QSPUTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, QSPU_PORT));
        }
    }
}
...
if (node)
  {
    m_isGateway = (node->GetId () == m_gatewayNodeId);
  }

m_igadTimer.SetFunction (&RoutingProtocol::IgadTimerExpire, this);
m_uavadTimer.SetFunction (&RoutingProtocol::UavadTimerExpire, this);

if (m_isGateway)
  {
    m_igadTimer.Schedule (MilliSeconds (100));
  }
else
  {
    m_uavadTimer.Schedule (Seconds (1.0));
  }

m_lastLoadUpdate = Simulator::Now ();
    Ipv4RoutingProtocol::DoInitialize();
}
Vector
RoutingProtocol::GetNodePosition () const
{
  Ptr<Node> node = m_ipv4->GetObject<Node> ();
  if (!node)
    {
...
Vector
RoutingProtocol::GetNodeVelocity () const
{
  Ptr<Node> node = m_ipv4->GetObject<Node> ();
  if (!node)
    {
...
Ipv4Address
RoutingProtocol::GetMainAddress () const
{
  for (auto const &j : m_socketAddresses)
    {
...
bool
RoutingProtocol::IsDuplicateIgad (Ipv4Address origin, uint32_t id)
{
  auto key = std::make_pair (origin, id);
  auto it = m_igadCache.find (key);

  if (it != m_igadCache.end ())
    {
      return true;
    }

  m_igadCache[key] = Simulator::Now ();
  return false;
}

bool
RoutingProtocol::IsDuplicateUavad (Ipv4Address origin, uint32_t id)
{
  auto key = std::make_pair (origin, id);
  auto it = m_uavadCache.find (key);

  if (it != m_uavadCache.end ())
    {
      return true;
    }

  m_uavadCache[key] = Simulator::Now ();
  return false;
}

uint32_t
RoutingProtocol::ComputeConnectivity10000 (double distance) const
{
  if (distance >= m_txRange)
    {
      return 1;
    }

  double co = 1.0 - (distance / m_txRange);
  co = std::max (0.0001, std::min (1.0, co));

  return static_cast<uint32_t> (std::round (co * 10000.0));
}

Time
RoutingProtocol::ComputeLinkLifetime (Vector p1, Vector v1, Vector p2, Vector v2) const
{
  Vector dp (p2.x - p1.x, p2.y - p1.y, p2.z - p1.z);
  Vector dv (v2.x - v1.x, v2.y - v1.y, v2.z - v1.z);

  double a = dv.x * dv.x + dv.y * dv.y + dv.z * dv.z;
  double b = 2.0 * (dp.x * dv.x + dp.y * dv.y + dp.z * dv.z);
  double c = dp.x * dp.x + dp.y * dp.y + dp.z * dp.z - m_txRange * m_txRange;
  ...
  return Seconds (std::min (t2, 3600.0));
}

Time
RoutingProtocol::EstimateQueueDelay (uint32_t packetSizeBytes) const
{
  double serviceBps = m_dataRateBps * 0.5;
  double packetBits = packetSizeBytes * 8.0;

  if (m_loadBps >= serviceBps * 0.95)
    {
      return Seconds (1.0);
    }

  double delay = packetBits / std::max (serviceBps - m_loadBps, 1.0);
  return Seconds (delay);
}

Time
RoutingProtocol::ComputeLinkDelay (Vector p1, Vector p2, uint32_t packetSizeBytes) const
{
  double dx = p1.x - p2.x;
  double dy = p1.y - p2.y;
  double dz = p1.z - p2.z;
  ...
  Time propagation = Seconds (distance / 299792458.0);
  Time transmission = Seconds ((packetSizeBytes * 8.0) / m_dataRateBps);
  Time queue = EstimateQueueDelay (packetSizeBytes);

  return propagation + transmission + queue;
}

uint32_t
RoutingProtocol::ComputeCost10000 (uint32_t co10000, Time lifetime, Time delay) const
{
  double co = std::max (co10000 / 10000.0, 0.0001);

  double lRef = 10.0;
  double dRef = 0.1;

  double lNorm = std::min (lifetime.GetSeconds () / lRef, 1.0);
  lNorm = std::max (lNorm, 0.0001);

  double dNorm = delay.GetSeconds () / dRef;

  double cost = m_alpha * (1.0 / co)
              + m_beta * (1.0 / lNorm)
              + m_gamma * dNorm;

  return static_cast<uint32_t> (std::round (cost * 10000.0));
}

void
RoutingProtocol::UpdateLoadEstimate (uint32_t bytes)
{
  m_windowBytes += bytes;

  Time now = Simulator::Now ();
  double dt = (now - m_lastLoadUpdate).GetSeconds ();

  if (dt >= m_loadWindow.GetSeconds ())
    {
      double instantBps = (m_windowBytes * 8.0) / std::max (dt, 1e-6);

      m_loadBps = (1.0 - m_loadEwmaAlpha) * m_loadBps
                + m_loadEwmaAlpha * instantBps;

      m_windowBytes = 0;
      m_lastLoadUpdate = now;
    }
}
bool
RoutingProtocol::UpdateQspuRoute (Ipv4Address destination,
                                  Ipv4Address nextHop,
                                  Ipv4Address receiver,
                                  uint8_t hop,
                                  uint32_t co10000,
                                  ...
                                  uint32_t cost10000)
{
  ...
  RoutingTableEntry newEntry (dev,
                              ...
                              hop,
                              nextHop,
                              std::max (m_activeRouteTimeout, lifetime));

  newEntry.SetQspuConnectivity10000 (co10000);
  newEntry.SetQspuLifetime (lifetime);
  newEntry.SetQspuDelay (delay);
  newEntry.SetQspuCost10000 (cost10000);
  newEntry.SetQspuRoute (true);

  RoutingTableEntry oldEntry;
  if (!m_routingTable.LookupRoute (destination, oldEntry))
    {
      m_routingTable.AddRoute (newEntry);
      return true;
    }

  if (oldEntry.GetFlag () != VALID)
    {
      m_routingTable.Update (newEntry);
      return true;
    }

  if (!oldEntry.IsQspuRoute ())
    {
      m_routingTable.Update (newEntry);
      return true;
    }

  // Hysteresis: prihvati samo ako je nova ruta barem 10% bolja.
  if (cost10000 * 100 < oldEntry.GetQspuCost10000 () * 90)
    {
      m_routingTable.Update (newEntry);
      return true;
    }
...
void
RoutingProtocol::IgadTimerExpire ()
{
  if (!m_isGateway)
    {
      return;
  ...
  SendIgad ();
  m_igadTimer.Schedule (m_igadInterval);
}

void
RoutingProtocol::SendIgad ()
{
  if (!m_isGateway)
    {
      return;
  ...
  Ipv4Address myAddress = GetMainAddress ();
  Vector myPos = GetNodePosition ();
  Vector myVel = GetNodeVelocity ();
  ...
  for (auto j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      QspuAdHeader ad;
      ad.SetOrigin (iface.GetLocal ());
      ad.SetGateway (iface.GetLocal ());
      ad.SetAdId (++m_igadId);
      ad.SetHopCount (0);
      ad.SetConnectivity10000 (10000);
      ad.SetLifetimeMs (3600 * 1000);
      ad.SetDelayUs (0);
      ad.SetLastPosition (myPos);
      ad.SetLastVelocity (myVel);

      Ptr<Packet> packet = Create<Packet> ();
      packet->AddHeader (ad);

      TypeHeader tHeader (QSPUTYPE_IGAD);
      packet->AddHeader (tHeader);
...
void
RoutingProtocol::RecvIgad (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  QspuAdHeader ad;
  p->RemoveHeader (ad);

  if (IsMyOwnAddress (ad.GetOrigin ()))
    {
      return;
    }

  if (IsDuplicateIgad (ad.GetOrigin (), ad.GetAdId ()))
    {
      return;
    }
  ...
  Vector prevPos = ad.GetLastPosition ();
  Vector prevVel = ad.GetLastVelocity ();

  double dx = myPos.x - prevPos.x;
  double dy = myPos.y - prevPos.y;
  double dz = myPos.z - prevPos.z;
  double distance = std::sqrt (dx * dx + dy * dy + dz * dz);

  uint32_t linkCo = ComputeConnectivity10000 (distance);
  Time linkLifetime = ComputeLinkLifetime (myPos, myVel, prevPos, prevVel);
  Time linkDelay = ComputeLinkDelay (myPos, prevPos, p->GetSize ());

  uint32_t oldCo = ad.GetConnectivity10000 ();
  uint32_t newCo = std::min (oldCo, linkCo);

  Time oldLifetime = MilliSeconds (ad.GetLifetimeMs ());
  Time newLifetime = std::min (oldLifetime, linkLifetime);

  Time oldDelay = MicroSeconds (ad.GetDelayUs ());
  Time newDelay = oldDelay + linkDelay;

  uint8_t newHop = ad.GetHopCount () + 1;

  if (newHop > m_maxHopCount)
    {
      return;
    }

  if (newCo < m_minConnectivity10000)
    {
      return;
    }

  if (newLifetime < m_minLifetime)
    {
      return;
    }

  if (newDelay > m_maxDelay)
    {
      return;
    }

  uint32_t cost = ComputeCost10000 (newCo, newLifetime, newDelay);

  bool updated = UpdateQspuRoute (ad.GetGateway (),
                                  sender,
                                  receiver,
                                  newHop,
                                  newCo,
  ...
  m_currentGateway = ad.GetGateway ();

  // Rebroadcast IGAD dalje kroz mrežu.
  ad.SetHopCount (newHop);
  ad.SetConnectivity10000 (newCo);
  ad.SetLifetimeMs (static_cast<uint32_t> (std::min (newLifetime.GetMilliSeconds (), int64_t (3600 * 1000))));
  ad.SetDelayUs (static_cast<uint32_t> (std::min (newDelay.GetMicroSeconds (), int64_t (3600LL * 1000000LL))));
  ad.SetLastPosition (myPos);
  ad.SetLastVelocity (myVel);
...
void
RoutingProtocol::UavadTimerExpire ()
{
  if (!m_isGateway)
    {
      SendUavad ();
      m_uavadTimer.Schedule (m_uavadInterval);
    }
}

void
RoutingProtocol::SendUavad ()
{
  if (m_isGateway)
    {
      return;
  ...
  if (m_currentGateway == Ipv4Address ())
    {
      return;
    }

  RoutingTableEntry gwRoute;
  if (!m_routingTable.LookupValidRoute (m_currentGateway, gwRoute))
    {
      return;
    }

  Ipv4Address myAddress = GetMainAddress ();
  if (myAddress == Ipv4Address ())
    {
      return;
    }
  ...
  QspuAdHeader ad;
  ad.SetOrigin (myAddress);
  ad.SetGateway (m_currentGateway);
  ad.SetAdId (++m_uavadId);
  ad.SetHopCount (0);
  ad.SetConnectivity10000 (10000);
  ad.SetLifetimeMs (3600 * 1000);
  ad.SetDelayUs (0);
  ...
  TypeHeader tHeader (QSPUTYPE_UAVAD);
  packet->AddHeader (tHeader);

  Ptr<Socket> socket = FindSocketWithInterfaceAddress (gwRoute.GetInterface ());
  if (!socket)
    {
      return;
    }
  ...
  socket->SendTo (packet,
                  0,
                  InetSocketAddress (gwRoute.GetNextHop (), QSPU_PORT));
}

void
RoutingProtocol::RecvUavad (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
  QspuAdHeader ad;
  p->RemoveHeader (ad);
  ...
  if (IsDuplicateUavad (ad.GetOrigin (), ad.GetAdId ()))
    {
      return;
    }
  ...
  uint32_t newCo = std::min (ad.GetConnectivity10000 (), linkCo);
  Time newLifetime = std::min (MilliSeconds (ad.GetLifetimeMs ()), linkLifetime);
  Time newDelay = MicroSeconds (ad.GetDelayUs ()) + linkDelay;
  uint8_t newHop = ad.GetHopCount () + 1;
  ...
  // Reverse ruta: do origin UAV-a idem preko sendera.
  UpdateQspuRoute (ad.GetOrigin (),
                   sender,
                   receiver,
                   newHop,
                   newCo,
  ...
  // Ako sam gateway kojem je UAVAD namijenjen, završavam.
  if (IsMyOwnAddress (ad.GetGateway ()))
    {
      return;
    }

  // Inače proslijedi UAVAD dalje prema gateway-u.
  RoutingTableEntry gwRoute;
  if (!m_routingTable.LookupValidRoute (ad.GetGateway (), gwRoute))
    {
      return;
    }
