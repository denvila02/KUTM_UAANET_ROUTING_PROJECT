...
enum MessageType
{
    QSPUTYPE_RREQ = 1,    //!< QSPUTYPE_RREQ
    QSPUTYPE_RREP = 2,    //!< QSPUTYPE_RREP
    QSPUTYPE_RERR = 3,    //!< QSPUTYPE_RERR
    QSPUTYPE_RREP_ACK = 4, //!< QSPUTYPE_RREP_ACK
    QSPUTYPE_IGAD = 5, 
    QSPUTYPE_UAVAD = 6
};
...
class TypeHeader : public Header
{
  public:
    /**
     * constructor
     * \param t the QSPU RREQ type
     */
    TypeHeader(MessageType t = QSPUTYPE_RREQ);
...
// Koristi se i za IGAD i za UAVAD.
//-----------------------------------------------------------------------------

class QspuAdHeader : public Header
{
public:
  QspuAdHeader ();
  ...
  void SetAdId (uint32_t id)
  {
    m_adId = id;
  }

  uint32_t GetAdId () const
  {
    return m_adId;
  }

  void SetHopCount (uint8_t hop)
  {
    m_hopCount = hop;
  }
  ...
  void SetConnectivity10000 (uint32_t co)
  {
    m_connectivity10000 = co;
  }

  uint32_t GetConnectivity10000 () const
  {
    return m_connectivity10000;
  }

  void SetLifetimeMs (uint32_t lifetimeMs)
  {
    m_lifetimeMs = lifetimeMs;
  }

  uint32_t GetLifetimeMs () const
  {
    return m_lifetimeMs;
  }
  ...
  uint32_t GetDelayUs () const
  {
    return m_delayUs;
  }

  void SetLastPosition (Vector p)
  {
    m_lastXcm = static_cast<int32_t> (std::round (p.x * 100.0));
    m_lastYcm = static_cast<int32_t> (std::round (p.y * 100.0));
    m_lastZcm = static_cast<int32_t> (std::round (p.z * 100.0));
  }

  Vector GetLastPosition () const
  {
    return Vector (m_lastXcm / 100.0, m_lastYcm / 100.0, m_lastZcm / 100.0);
  }

  void SetLastVelocity (Vector v)
  {
    m_lastVxCms = static_cast<int32_t> (std::round (v.x * 100.0));
    m_lastVyCms = static_cast<int32_t> (std::round (v.y * 100.0));
    m_lastVzCms = static_cast<int32_t> (std::round (v.z * 100.0));
  }

  Vector GetLastVelocity () const
  {
    return Vector (m_lastVxCms / 100.0, m_lastVyCms / 100.0, m_lastVzCms / 100.0);
  }

  bool operator== (QspuAdHeader const &o) const;
...
std::ostream & operator<< (std::ostream & os, QspuAdHeader const & h);
