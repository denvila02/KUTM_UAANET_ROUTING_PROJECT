 ...
 */
#ifndef QSPUROUTINGPROTOCOL_H
#define QSPUROUTINGPROTOCOL_H

#include "qspu-dpd.h"
#include "qspu-neighbor.h"
#include "qspu-packet.h"
#include "qspu-rqueue.h"
#include "qspu-rtable.h"
...
class RoutingProtocol : public Ipv4RoutingProtocol
{
  public:
    ...
    static const uint32_t QSPU_PORT;
    ...
    void SetGatewayNodeId (uint32_t id)
{
  m_gatewayNodeId = id;
}
...
void SetIgadInterval (Time t)
{
  m_igadInterval = t;
}

Time GetIgadInterval () const
{
  return m_igadInterval;
}

void SetUavadInterval (Time t)
{
  m_uavadInterval = t;
}

Time GetUavadInterval () const
{
  return m_uavadInterval;
}
...
uint32_t m_minConnectivity10000;
Time m_minLifetime;
Time m_maxDelay;
uint8_t m_maxHopCount;

bool m_dropZeroVelocityUavs;

Ipv4Address m_currentGateway;

std::map<std::pair<Ipv4Address, uint32_t>, Time> m_igadCache;
std::map<std::pair<Ipv4Address, uint32_t>, Time> m_uavadCache;

uint64_t m_windowBytes;
Time m_loadWindow;
Time m_lastLoadUpdate;
double m_loadBps;
double m_loadEwmaAlpha;
  ...
  private:
     ...
     */
    void RecvQspu(Ptr<Socket> socket);
    /**
     * Receive RREQ
     * \param p packet
     ...
     */
     void SendIgad ();
void RecvIgad (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender);
void IgadTimerExpire ();

void SendUavad ();
void RecvUavad (Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender);
void UavadTimerExpire ();

Vector GetNodePosition () const;
Vector GetNodeVelocity () const;
Ipv4Address GetMainAddress () const;

bool IsDuplicateIgad (Ipv4Address origin, uint32_t id);
bool IsDuplicateUavad (Ipv4Address origin, uint32_t id);

uint32_t ComputeConnectivity10000 (double distance) const;
Time ComputeLinkLifetime (Vector p1, Vector v1, Vector p2, Vector v2) const;
Time ComputeLinkDelay (Vector p1, Vector p2, uint32_t packetSizeBytes) const;
Time EstimateQueueDelay (uint32_t packetSizeBytes) const;
uint32_t ComputeCost10000 (uint32_t co10000, Time lifetime, Time delay) const;

void UpdateLoadEstimate (uint32_t bytes);
bool UpdateQspuRoute (Ipv4Address destination,
                      Ipv4Address nextHop,
