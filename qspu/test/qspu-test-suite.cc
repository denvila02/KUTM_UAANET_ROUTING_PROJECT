 ...
 */
#include "ns3/qspu-neighbor.h"
#include "ns3/qspu-packet.h"
#include "ns3/qspu-rqueue.h"
#include "ns3/qspu-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("QSPU TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(QSPUTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), QSPUTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(QSPUTYPE_RREP);
 ...
 * \ingroup qspu-test
 *
 * \brief Unit test for QSPU routing table entry
 */
...
struct QspuRqueueTest : public TestCase
{
    QspuRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
QspuRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&QspuRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&QspuRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &QspuRqueueTest::CheckTimeout, this);
 ...
 * \brief Unit test for QSPU routing table entry
 */
struct QspuRtableEntryTest : public TestCase
{
    QspuRtableEntryTest()
 ...
 * \ingroup qspu-test
 *
 * \brief Unit test for QSPU routing table
 */
struct QspuRtableTest : public TestCase
{
    QspuRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup qspu-test
 *
 * \brief QSPU test suite
 */
class QspuTestSuite : public TestSuite
{
  public:
    QspuTestSuite()
        : TestSuite("routing-qspu", Type::UNIT)
    {
        ...
        AddTestCase(new QspuRqueueTest, TestCase::Duration::QUICK);
        AddTestCase(new QspuRtableEntryTest, TestCase::Duration::QUICK);
        AddTestCase(new QspuRtableTest, TestCase::Duration::QUICK);
    }
} g_qspuTestSuite; ///< the test suite
