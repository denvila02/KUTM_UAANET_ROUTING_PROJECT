...
#include "qspu-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/qspu-helper.h"
 ...
 * \ingroup qspu-test
 *
 * \brief QSPU regression test suite
 */
class QspuRegressionTestSuite : public TestSuite
{
  public:
    QspuRegressionTestSuite()
        : TestSuite("routing-qspu-regression", Type::SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("qspu-chain-regression-test"),
...
} g_qspuRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("QSPU chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & QSPU
    QspuHelper qspu; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(qspu);
    ...
    streamsUsed += qspu.AssignStreams(*m_nodes, streamsUsed);
    // QSPU uses m_size more streams
