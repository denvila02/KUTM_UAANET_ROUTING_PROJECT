/* Opis: Simulacija AODV rutiranja sa RandomWaypoint mobility modelom
...
int main(int argc, char *argv[])
{
 ...
 double nodeSpeed = 20.0; // m/s (brzina UAV-a ili maksimalna brzina, zavisno od fixedNodeSpeed)
 double nodeMinSpeed = 10.0; // m/s (minimalna brzina za UniformRandomVariable)
 bool fixedNodeSpeed = false; // true => svi UAV-ovi koriste konstantnu brzinu nodeSpeed
...
// std::string txtFileName = "aodv_results.txt";
 ...
 cmd.AddValue("nodeSpeed", "Brzina UAV-a ako je fixedNodeSpeed=true; inace maksimalna brzina UAV-a (m/s)", nodeSpeed);
 cmd.AddValue("nodeMinSpeed", "Minimalna brzina UAV-a za UniformRandomVariable (m/s)", nodeMinSpeed);
 cmd.AddValue("fixedNodeSpeed", "Ako je true, RandomWaypoint koristi konstantnu brzinu nodeSpeed za sve UAV-ove", fixedNodeSpeed);
 ...
 if (!fixedNodeSpeed && nodeSpeed < nodeMinSpeed)
 {
     NS_FATAL_ERROR("nodeSpeed mora biti >= nodeMinSpeed kada je fixedNodeSpeed=false");
 }
 ...
 if (fixedNodeSpeed)
 {
     NS_LOG_UNCOND("Brzina UAV: konstantno " << nodeSpeed << " m/s");
 }
 else
 {
     NS_LOG_UNCOND("Brzina UAV: " << nodeMinSpeed << " - " << nodeSpeed << " m/s");
 }
...
NetDeviceContainer devices = wifi.Install(wifiPhy, wifiMac, allNodes);
 ...
 // 3. Mobility - RandomWaypoint (UAV kretanje)
 ...
 // 3.2 UAV mobility - 3D RandomWaypoint
 Ptr<RandomBoxPositionAllocator> uavPositionAlloc = CreateObject<RandomBoxPositionAllocator>();
 ...
 uavPositionAlloc->SetAttribute("X", PointerValue(xVal));
 uavPositionAlloc->SetAttribute("Y", PointerValue(yVal));
 uavPositionAlloc->SetAttribute("Z", PointerValue(zVal));
 
 MobilityHelper uavMobility;
 uavMobility.SetPositionAllocator(uavPositionAlloc);
 
 std::string speedRandomVariable;
 if (fixedNodeSpeed)
 {
     speedRandomVariable = "ns3::ConstantRandomVariable[Constant=" +
         std::to_string(nodeSpeed) + "]";
 }
 else
 {
     speedRandomVariable = "ns3::UniformRandomVariable[Min=" +
         std::to_string(nodeMinSpeed) + "|Max=" +
         std::to_string(nodeSpeed) + "]";
 }

 uavMobility.SetMobilityModel("ns3::RandomWaypointMobilityModel",
     "Speed", StringValue(speedRandomVariable),
...
{
    csvFile
        << "protocol,run,nGcs,nUavs,nNodes,simTime,areaSize,txRange,nodeSpeed,nodeMinSpeed,fixedNodeSpeed,nodePause,"
