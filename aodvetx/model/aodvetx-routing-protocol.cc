 ...
 *
 * Based on
 *      NS-2 AODVETX model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      AODVETX-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/AODVETX-UU
 *
...
NS_LOG_COMPONENT_DEFINE("AodvEtxRoutingProtocol");

namespace aodvetx
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for AODVETX control traffic
const uint32_t RoutingProtocol::AODVETX_PORT = 654;

/**
 * \ingroup aodvetx
 * \brief Tag used by AODVETX implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::aodvetx::DeferredRouteOutputTag")
                                .SetParent<Tag>()
                                .SetGroupName("AodvEtx")
...
{
    static TypeId tid =
        TypeId("ns3::aodvetx::RoutingProtocol")
            ...
            .AddAttribute("EnableBroadcast",
                          ...
                          MakeBooleanChecker())
            .AddAttribute("EnableEtx",
                          "Enable ETX metric", 
                          BooleanValue(true), 
                          MakeBooleanAccessor(&RoutingProtocol::SetEtxEnable,
                          	              &RoutingProtocol::GetEtxEnable),
                          MakeBooleanChecker())
            .AddAttribute("LppInterval",
                          "Link probe packet emission interval", 
                          TimeValue(Seconds(1)), 
...
RoutingProtocol::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    *stream->GetStream() << "Node: " << m_ipv4->GetObject<Node>()->GetId()
                         ...
                         << ", AODVETX Routing table" << std::endl;
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No aodvetx interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // AODVETX is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == AODVETX_PORT)
                    {
                        // AODVETX packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("AODVETX does not work with more then one address per each interface.");
    }
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodvEtx, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), AODVETX_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAodvEtx, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), AODVETX_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("AODVETX does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in AODVETX operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when AODVETX does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // AODVETX needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available AODVETX interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid AODVETX source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by aodvetx
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(AODVETXTYPE_RREQ);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, AODVETX_PORT));
}
...
RoutingProtocol::ScheduleRreqRetry(Ipv4Address dst)
{
    ...
    else
    {
        ...
        if (backoffFactor>=30){
        	NS_LOG_WARN("AODVETX: backoffFactor too large ("<<backoffFactor<<") for destination "<< dst<< ". Cancelling RREQ retry to avoid integer overflow.");
...
void
RoutingProtocol::RecvAodvEtx(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("AODVETX node " << this << " received a AODVETX packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("AODVETX message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    ...
    case AODVETXTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    case AODVETXTYPE_LPP: {
...
RoutingProtocol::UpdateRouteToNeighbor(Ipv4Address sender, Ipv4Address receiver)
{
    ...
    if (!m_routingTable.LookupRoute(sender, toNeighbor))
    {
        ...
        RoutingTableEntry newEntry(
            ...
            m_nbEtx.GetEtxForNeighbor(sender));
        m_routingTable.AddRoute(newEntry);
    }
    else
    {
        ...
        else
        {
            RoutingTableEntry newEntry(
                ...
                /*hops=*/1,
                /*nextHop=*/sender,
                /*lifetime=*/std::max(m_activeRouteTimeout, toNeighbor.GetLifeTime()),m_nbEtx.GetEtxForNeighbor(sender));
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    uint32_t etx = m_nbEtx.GetEtxForNeighbor(src);
    if (etx == NeighborEtx::EtxMaxValue()){rreqHeader.SetEtx(etx);}
    else{rreqHeader.SetEtx(rreqHeader.GetEtx()+etx);}
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(AODVETXTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), AODVETX_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(AODVETXTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), AODVETX_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(AODVETXTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, AODVETX_PORT));
}
...
RoutingProtocol::RecvReply(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address sender)
{
    ...
    uint32_t etx = m_nbEtx.GetEtxForNeighbor (sender);
...
if (etx == NeighborEtx::EtxMaxValue ())
  ...
  {
    rrepHeader.SetEtx (rrepHeader.GetEtx () + etx);
  }
    /*
...
RoutingProtocol::ProcessHello(const RrepHeader& rrepHeader, Ipv4Address receiver)
{
    ...
    if (!m_routingTable.LookupRoute(rrepHeader.GetDst(), toNeighbor))
    {
        ...
        RoutingTableEntry newEntry(
            ...
            /*lifetime=*/rrepHeader.GetLifeTime(),
            m_nbEtx.GetEtxForNeighbor(rrepHeader.GetDst()));
        m_routingTable.AddRoute(newEntry);
    }
    else
    {
        ...
        toNeighbor.SetEtx(m_nbEtx.GetEtxForNeighbor(rrepHeader.GetDst()));
        m_routingTable.Update(toNeighbor);
    }
    if (m_enableHello)
    {
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(AODVETXTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
  ...
  for (auto j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
    {
      ...
      TypeHeader tHeader (AODVETXTYPE_LPP);
      packet->AddHeader (tHeader);
      ...
      Time jitter = MicroSeconds (m_uniformRandomVariable->GetInteger (0, 10000));
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(AODVETXTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, AODVETX_PORT));
        }
    }
}
