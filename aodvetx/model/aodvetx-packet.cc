...
{
    static TypeId tid = TypeId("ns3::aodvetx::TypeHeader")
                            .SetParent<Header>()
...
{
    static TypeId tid = TypeId("ns3::aodvetx::RreqHeader")
                            .SetParent<Header>()
                            .SetGroupName("AodvEtx")
...
{
    static TypeId tid = TypeId("ns3::aodvetx::RrepHeader")
                            .SetParent<Header>()
                            .SetGroupName("AodvEtx")
...
{
    static TypeId tid = TypeId("ns3::aodvetx::RrepAckHeader")
                            .SetParent<Header>()
                            .SetGroupName("AodvEtx")
...
{
    static TypeId tid = TypeId("ns3::aodvetx::RerrHeader")
                            .SetParent<Header>()
                            .SetGroupName("AodvEtx")
...
LppHeader::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::aodvetx::LppHeader")
    .SetParent<Header> ()
    .SetGroupName ("Aodvetx")
...
LppHeader::Serialize (Buffer::Iterator i) const
{
  ...
  i.WriteHtonU32 (m_originSeqno);
  i.WriteU8 (static_cast<uint8_t> (GetNumberNeighbors ()));

  for (auto j = m_neighborsLppCnt.begin (); j != m_neighborsLppCnt.end (); ++j)
    {
      WriteTo (i, j->first);
      i.WriteU8 (j->second);
    }
...
{
  os << "LPP ID: " << static_cast<uint32_t> (m_lppId)
     << " Originator IP address: " << m_originAddr
     << " Originator Sequence number: " << m_originSeqno
     << " Number of neighbors: " << GetNumberNeighbors ();
}
...
LppHeader::AddToNeighborsList (Ipv4Address neighbor, uint8_t lppCnt)
{
  ...
  NS_ASSERT (GetNumberNeighbors () < 255);
...
LppHeader::RemoveFromNeighborsList (std::pair<Ipv4Address, uint8_t> &un)
{
  ...
  auto i = m_neighborsLppCnt.begin ();
  un = *i;
...
{
  return m_lppId == o.m_lppId
         && m_originAddr == o.m_originAddr
         && m_originSeqno == o.m_originSeqno
         && m_neighborsLppCnt == o.m_neighborsLppCnt;
}
