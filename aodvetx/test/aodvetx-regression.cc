...
#include "aodvetx-regression.h"

#include "bug-772.h"

#include "ns3/abort.h"
#include "ns3/aodvetx-helper.h"
 ...
 *
 * \brief AODVETX regression test suite
 */
class AodvEtxRegressionTestSuite : public TestSuite
{
  public:
    AodvEtxRegressionTestSuite()
        : TestSuite("routing-aodvetx-regression", Type::SYSTEM)
    {
        SetDataDir(NS_TEST_SOURCEDIR);
        // General RREQ-RREP-RRER test case
        AddTestCase(new ChainRegressionTest("aodvetx-chain-regression-test"),
...
} g_aodvetxRegressionTestSuite; ///< the test suite
...
ChainRegressionTest::ChainRegressionTest(const char* const prefix,
    ...
    : TestCase("AODVETX chain regression test"),
      m_nodes(nullptr),
      m_prefix(prefix),
      m_time(t),
      m_size(size),
...
ChainRegressionTest::CreateDevices()
{
    ...
    // 2. Setup TCP/IP & AODVETX
    AodvEtxHelper aodvetx; // Use default parameters here
    InternetStackHelper internetStack;
    internetStack.SetRoutingHelper(aodvetx);
    ...
    streamsUsed += aodvetx.AssignStreams(*m_nodes, streamsUsed);
    // AODVETX uses m_size more streams
