 ...
 */
#include "ns3/aodvetx-neighbor.h"
#include "ns3/aodvetx-packet.h"
#include "ns3/aodvetx-rqueue.h"
#include "ns3/aodvetx-rtable.h"
...
{
    TypeHeaderTest()
        : TestCase("AODVETX TypeHeader")
    {
    }

    void DoRun() override
    {
        TypeHeader h(AODVETXTYPE_RREQ);
        NS_TEST_EXPECT_MSG_EQ(h.IsValid(), true, "Default header is valid");
        NS_TEST_EXPECT_MSG_EQ(h.Get(), AODVETXTYPE_RREQ, "Default header is RREQ");

        Ptr<Packet> p = Create<Packet>();
        p->AddHeader(h);
        TypeHeader h2(AODVETXTYPE_RREP);
 ...
 * \ingroup aodvetx-test
 *
 * \brief Unit test for AODVETX routing table entry
 */
...
struct AodvEtxRqueueTest : public TestCase
{
    AodvEtxRqueueTest()
        : TestCase("Rqueue"),
          q(64, Seconds(30))
...
AodvEtxRqueueTest::DoRun()
{
    ...
    h.SetDestination(Ipv4Address("1.2.3.4"));
    h.SetSource(Ipv4Address("4.3.2.1"));
    Ipv4RoutingProtocol::UnicastForwardCallback ucb = MakeCallback(&AodvEtxRqueueTest::Unicast, this);
    Ipv4RoutingProtocol::ErrorCallback ecb = MakeCallback(&AodvEtxRqueueTest::Error, this);
    ...
    Simulator::Schedule(q.GetQueueTimeout() + Seconds(1), &AodvEtxRqueueTest::CheckTimeout, this);
...
void
AodvEtxRqueueTest::CheckSizeLimit()
{
...
void
AodvEtxRqueueTest::CheckTimeout()
{
 ...
 * \brief Unit test for AODVETX routing table entry
 */
struct AodvEtxRtableEntryTest : public TestCase
{
    AodvEtxRtableEntryTest()
 ...
 * \ingroup aodvetx-test
 *
 * \brief Unit test for AODVETX routing table
 */
struct AodvEtxRtableTest : public TestCase
{
    AodvEtxRtableTest()
        : TestCase("Rtable")
    {
 ...
 * \ingroup aodvetx-test
 *
 * \brief AODVETX test suite
 */
class AodvEtxTestSuite : public TestSuite
{
  public:
    AodvEtxTestSuite()
        : TestSuite("routing-aodvetx", Type::UNIT)
    {
        ...
        AddTestCase(new AodvEtxRqueueTest, TestCase::Duration::QUICK);
        AddTestCase(new AodvEtxRtableEntryTest, TestCase::Duration::QUICK);
        AddTestCase(new AodvEtxRtableTest, TestCase::Duration::QUICK);
    }
} g_aodvetxTestSuite; ///< the test suite
